...
class CommonSdk
{
    ...
    protected function send($params, $method, $type = 'get')
    {
        ...
        $encryptParams = $type == 'post' ? $requestData['data'] : $this->baseParams['data'];
        $paramsAndOriginResponseData = 'params------'.json_encode($params).'-----afterEncrypt------'.$encryptParams.'----callback-----'.$data;
        Config::setOriginResponseData($paramsAndOriginResponseData);
