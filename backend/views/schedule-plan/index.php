...
<div class="layui-row">
    <form class="layui-form" action="">
        <div class="layui-form-item layui-inline">
            ...
            <div class="layui-input-block" style="width:100px;margin-left:100px;">
                <select name="is_done">
                    ...
                    <option value="0" <?php if(trim($request->get('is_done')) == "0"){ echo 'selected="selected"'; }?>><?php echo '未执行'; ?></option>
                    <option value="1" <?php if(trim($request->get('is_done')) == "1"){ echo 'selected="selected"'; }?>><?php echo '已执行'; ?></option>
                </select>
            </div>
        </div>
                ...
                <select name="min_department_id" lay-search="" lay-filter="search_fkid"  class="search_fkid">
                    ...
                    <?php if(!empty($department)): ?>
                        <?php foreach($department as $key => $value):?>
                            <option value="<?=$key?>" <?php if($key == ($requestParams['min_department_id']??0)){ echo 'selected="selected"'; }?>><?=Html::encode($value)?></option>
                        <?php endforeach;?>
                    <?php endif;?>
                </select>
            </div>
        ...
        <div class="layui-form-item layui-inline">
            <label class="layui-form-label" style="width:200px;">医院/医生名称：</label>
            <div class="layui-input-block layui-inline" style="margin-left:0px;width:280px;">
                <input type="text" name="keyword"  placeholder="请输入医院、医生名称搜索" autocomplete="off" class="layui-input" value="<?php echo Html::encode($request->get('keyword', '')); ?>">
            </div>
        </div>
        ...
        <?php if (!empty($dataProvider)): ?>
            <?php foreach ($dataProvider as $value): ?>
                <tr>
                    ...
                    <td ><?php echo Html::encode($value['min_doctor_name']); ?></td>
                    ...
                    <td ><?php echo $value['section_type_desc']; ?></td>
                    <td ><?php echo $value['visit_cost_desc']; ?></td>
                    <td ><?php echo $value['schedule_count']; ?></td>
                    <td ><?php echo $value['valid_date']; ?></td>
                    <td ><?php echo $value['done_status']; ?></td>
                </tr>
            <?php endforeach;?>
        <?php else: ?>
