...
class HospitalController extends CommonController
{
    ...
    public function actionDetail(){
        if (!$hospital_model = $this->findModel($this->user['min_hospital_id'])) {
    ...
    public function actionUpdate(){
        ...
        if (!$form_model = $this->findModel($this->user['min_hospital_id'])) {
            return $this->jsonError('未找到医院信息');
        }
        ...
        $exist = MinHospitalModel::find()
            ->where(['min_hospital_name' => $requestParams['min_hospital_name']])
            ->andWhere(['<>','min_hospital_id',$this->user['min_hospital_id']])
            ->exists();
        if ($exist){
        ...
        // 初审信息(民营医院是待审核)
        $form_model->check_status = MinHospitalModel::CHECK_STATUS_NORMAL;
        ...
        if ($log) {
            $log = rtrim($log,',');
            TbLog::addLog($log, '民营医院信息修改日志', ['admin_id' => $this->user['min_hospital_id'], 'admin_name' => $this->user['username']]);
        }
     ...
     * @date 2022-07-16
     */
    public function actionHospitalLevel(){
        $level_list = CommonFunc::$level_list;
        return $this->jsonSuccess($level_list);
    }
    ...
    public function actionSensitive()
    {
        ...
        if (!$model = $this->findModel($this->user['min_hospital_id'])) {
    ...
    public function findModel($hospital_id){
        ...
        return CreateForm::find()
            ->where(['min_hospital_id' => $hospital_id])
            ->select(join(',', $fields))
            ->limit(1)
            ->one();
    }
