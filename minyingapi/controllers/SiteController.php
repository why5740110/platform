...
class SiteController extends Controller
{
    ...
    public function actionLogin()
    {
        ...
        $model = new LoginForm();
        $model->login_type = LoginForm::HOSPITAL_LOGIN;
        ...
        $token_cache_key = sprintf(MinAccountModel::REDIS_TOKEN_KEY_HOSPITAL, $access_token);
        $expire_time = strtotime('today') + 86400 - time();
        // 返回用户基本信息
        $account = $model->getAccount();

        // 代理商id
        $agency_id = MinHospitalModel::find()->where(['min_hospital_id' => $account->enterprise_id])->select('agency_id')->scalar();
        $agency_info = MinAgencyModel::findOne(['agency_id' => $agency_id]);

        if (!$agency_info) {
            return $this->jsonError('未找到代理商信息');
        }
        ...
        $user_info = [
            ...
            'min_hospital_id' => $account->enterprise_id,
            'min_hospital_name' => $account->enterprise_name,
